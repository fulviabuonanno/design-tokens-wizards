...
async function generateAccessibilityReport(selectedProperties, tokens, outputsDir) {
  ...
  const reportPath = path.join(reportsDir, "accessibility-typography-report.md");
  
  let report = `# Typography Accessibility Report 🎨👁️\n\n`;
  ...
  report += `---\n\n`;
  report += `*Generated by Design Tokens Wizards - Typography Accessibility Report*\n`;
  report += `*Version 1.0.0*\n`;

  fs.writeFileSync(reportPath, report);
  
  console.log(chalk.bold.bgRedBright("\n======================================="));
  console.log(chalk.bold("📝 ACCESSIBILITY REPORT"));
  console.log(chalk.bold.bgRedBright("=======================================\n"));
  console.log(chalk.whiteBright(`✅ Generated accessibility report at:`));
  console.log(chalk.whiteBright(`   -> ${path.relative(process.cwd(), reportPath)}`));
}
