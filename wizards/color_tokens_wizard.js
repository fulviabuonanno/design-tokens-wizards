...
const askForInput = async (existingVariants = [], namingChoice = null, previousConcept = null) => {
  ...
  let generateRGB = await askYesNo("📌 Include color tokens in RGB format? (yes/no): ");
  let generateRGBA = await askYesNo("📌 Include color tokens RGBA format? (yes/no): ");
  let generateHSL = await askYesNo("📌 Include color tokens HSL format? (yes/no): ");
