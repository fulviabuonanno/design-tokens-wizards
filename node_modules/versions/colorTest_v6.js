...
const main = async () => {
  ...
  console.log(`Valid color detected: ${hex}`);

  // Siempre incluir HEX en los resultados
  colorTokens[hex].formats.HEX = color.toHexString();

  // Generación de formatos según las respuestas del usuario
  if (includeRGB) {
    colorTokens[hex].formats.RGB = color.toRgbString();
  }
  if (includeRGBA) {
    colorTokens[hex].formats.RGBA = color.setAlpha(0.5).toRgbString();
  }
  if (includeHSL) {
    colorTokens[hex].formats.HSL = color.toHslString();
  }
  ...
  stopNames.forEach(stopName => {
    ...
    if (stopColor) {
      ...
      if (includeRGB) {
        colorTokens[hex].stops[stopName].RGB = stopColor.toRgbString();
      }
      if (includeRGBA) {
        colorTokens[hex].stops[stopName].RGBA = stopColor.setAlpha(0.5).toRgbString();
      }
      if (includeHSL) {
        colorTokens[hex].stops[stopName].HSL = stopColor.toHslString();
      }
      colorTokens[hex].stops[stopName].HEX = stopColor.toHexString();  // Siempre incluir HEX
    } else {
